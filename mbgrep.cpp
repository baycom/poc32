...
int
CRegEx::regex_compile (char *source)
//*************************************************************************
//
//  Compile the pattern
//
//*************************************************************************
{
  ...
  while ((c = *s++) != 0) {
	...
	goto error;
      ...
      while (--spp > lp)	// Move pattern down

	spp[0] = spp[-1];	// one byte
...
char *
CRegEx::pmatch (char *begin, char *line, char *pattern)
//*************************************************************************
//
//
//
//*************************************************************************
{
  ...
  while ((op = *p++) != ENDPAT) {
    ...
    case PLUS:			// One or more ...
      ...
      l = pmatch (begin, l, p);
      if (l == NULL)
	goto notmatch;		// Gotta have a match
...
int
CRegEx::regex_match (char *pattern, char *line)
{
...
//*************************************************************************
{
  ...
  if (m_pattern[1] == TOUPPER)
    _strupr (lbuf + 1);

  if (m_pattern[0] == QUICK)
    return strstr (lbuf + 1, m_pattern + 2) != NULL;
  else {
    while (lbufp[0]) {
      if (pmatch (lbuf + 1, lbufp, m_pattern + 2))
	return 1;
      lbufp++;
    }
  }
...
//*************************************************************************
{
  ...
  while (fgets (obuf, LMAX - 2, fp)) {
    ++lno;
    m = regex_match (obuf);
    if ((m && !(m_options & o_v)) || (!m && (m_options & o_v))) {
      ++count;
      if (!(m_options & o_c)) {
	if (m_options & o_n)
	  putf ("%d:", lno);
...
int
grep (char *name, char *pattern, bitfeld options)
//*************************************************************************
//
//
//
//*************************************************************************
{
  ...
  if (name[0]) {
	...
	putf ("grep: can't open %s\n", name);
    }
    else
      putf ("bad regular expression.\n");
  }
  return matches;
}
